...
public class SourceFileDetector implements FileDetector {
     ...
     */
    public SourceFileDetector(Properties config) {
        this.config = config;
        this.supportedExtensions = loadSupportedExtensions();
        this.supportedFilenames = loadSupportedFilenames();
    }
    ...
    public void detect(NodeDecorator<ProjectFile> decorator) {
        ...
        try {
            decorator.enableTag(InspectorTags.TAG_SOURCE_FILE);

            // Also set file type specific tags for additional categorization
            String fileExtension = decorator.getNode().getFileExtension();
            if (fileExtension != null && !fileExtension.isEmpty()) {
                String normalizedExtension = fileExtension.startsWith(".") ? fileExtension.substring(1) : fileExtension;
                decorator.enableTag(normalizedExtension.toLowerCase() + ".source_type");
            }

        } catch (Exception e) {
            decorator.error("Error tagging source file: " + e.getMessage());
        }
    }
    ...
    public String getConfigurationSummary() {
        return String.format(
                "SourceFileDetector Configuration:\n" +
