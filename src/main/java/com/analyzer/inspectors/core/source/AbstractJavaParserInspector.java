...
public abstract class JavaParserInspector extends SourceFileInspector {
    ...
    /**
     * JavaParserInspector depends on both parent dependencies (source_file)
     * and Java-specific tags from InspectorTags.
     * 
     * This ensures JavaParser-based analysis only runs on files that have been:
     * 1. Identified as source files by SourceFileTagInspector
     * 2. Detected as Java language files
     * 3. Confirmed to have Java source code available
     * 
     * @param tags builder for collecting required dependencies
     */
    @Override
    public void depends(RequiredTags tags) {
        // Get parent dependencies (source_file)
        super.depends(tags);

        // Add Java-specific dependencies from InspectorTags
        tags.requires(InspectorTags.JAVA_DETECTED)
                .requires(InspectorTags.JAVA_IS_SOURCE)
                .requires(InspectorTags.LANGUAGE);
    }
}
