...
/**
 * Inspector that analyzes the number of interfaces implemented by Java classes using
 * multi-approach validation (ClassLoader + ASM + JavaParser).
 * 
 * <p>
 * This inspector uses runtime class loading as the primary analysis approach to count
 * both direct and inherited interface implementations. It provides comprehensive metrics
 * including total interface count, direct implementation count, and inherited interface count.
 * </p>
 * 
 * <p>
 * Key features:
 ...
 * 
 * <p>
 * Metrics are attached to the corresponding JavaClassNode with standardized property keys.
 * This enables architectural analysis and migration planning based on interface complexity.
 * </p>
 * 
...
@InspectorDependencies(
    ...
    produces = { "interfaces.total_count" }
)
public class InterfaceNumberInspector extends AbstractClassLoaderBasedInspector  {
    ...
    protected void analyzeLoadedClass(Class<?> loadedClass, ProjectFile projectFile, ProjectFileDecorator projectFileDecorator) {
        try {
            ...
            projectFileDecorator.setTag("interfaces.analyzed", "true");
            
            if (metrics.totalCount >= COMPLEX_INTERFACE_THRESHOLD) {
                projectFileDecorator.setTag("interfaces.pattern", "complex_implementation");
            }
            
            if (metrics.hasFrameworkInterfaces) {
                projectFileDecorator.setTag("interfaces.pattern", "framework_integration");
            }
     ...
     */
    private void attachMetricsToGraphNode(ProjectFile projectFile, InterfaceMetrics metrics, ProjectFileDecorator projectFileDecorator) {
        if (graphRepository == null) {
            ...
            if (fullyQualifiedName != null) {
                ...
                if (classNode != null) {
                    logger.debug("JavaClassNode found in graph for interface analysis: {}", fullyQualifiedName);
                    // Note: In future versions, we can extend JavaClassNode with public methods 
                    // for setting interface-specific properties
                } else {
                    logger.debug("JavaClassNode not found in graph for: {} - metrics stored as tags only", fullyQualifiedName);
                }
            }
        }
        
        // Store interface metrics as detailed tags
        projectFileDecorator.setTag(PROP_INTERFACES_TOTAL_COUNT, metrics.totalCount);
        projectFileDecorator.setTag(PROP_INTERFACES_DIRECT_COUNT, metrics.directCount);
        projectFileDecorator.setTag(PROP_INTERFACES_INHERITED_COUNT, metrics.inheritedCount);
        projectFileDecorator.setTag(PROP_INTERFACES_COMPLEXITY_SCORE, metrics.complexityScore);
        projectFileDecorator.setTag(PROP_INTERFACES_HAS_FRAMEWORK, metrics.hasFrameworkInterfaces);
        
        if (!metrics.frameworkTypes.isEmpty()) {
            projectFileDecorator.setTag(PROP_INTERFACES_FRAMEWORK_TYPES, String.join(",", metrics.frameworkTypes));
        }
        
        logger.debug("Stored interface metrics as tags: {} (total={}, direct={}, inherited={})", 
                    projectFile.getFullyQualifiedName(), metrics.totalCount, metrics.directCount, metrics.inheritedCount);
    }
