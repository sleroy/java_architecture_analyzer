...
# Script to add @AutoRegister annotation to all Inspector and Collector classes

# List of files to annotate
files=(
...
echo "Adding @AutoRegister annotation to ${#files[@]} files..."

for file in "${files[@]}"; do
  if [ -f "$file" ]; then
    # Check if already has @AutoRegister
    if grep -q "@AutoRegister" "$file"; then
      echo "  [SKIP] $file (already annotated)"
      continue
    fi
    
    # Add import if not present
    if ! grep -q "import com.analyzer.core.inspector.AutoRegister;" "$file"; then
      # Find the package line and add import after the last import
      sed -i '/^import/! b; $a\import com.analyzer.core.inspector.AutoRegister;' "$file"
    fi
    
    # Add @AutoRegister annotation before public class declaration
    sed -i '/^public class.*Inspector\|^public class.*Collector/ { 
      s/^public class/@AutoRegister\n&/
    }' "$file"
...
echo "Done! Annotated ${#files[@]} files."
