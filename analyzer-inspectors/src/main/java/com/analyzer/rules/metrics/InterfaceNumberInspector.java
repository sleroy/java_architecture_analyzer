 ...
 */
@InspectorDependencies(requires = { InspectorTags.TAG_JAVA_DETECTED,  InspectorTags.TAG_APPLICATION_CLASS}, produces = { "interfaces.total_count" })
public class InterfaceNumberInspector extends AbstractClassLoaderBasedInspector {
    ...
    protected void analyzeLoadedClass(Class<?> loadedClass, ProjectFile projectFile,
        ...
        try {
            ...
            attachMetricsToGraphNode(projectFile, metrics);
    ...
    private void attachMetricsToGraphNode(ProjectFile projectFile, InterfaceMetrics metrics) {
        ...
        // Use MAX strategy - identify the class with most interfaces in this file
        aggregateMaxMetric(projectFile, "interfaces.total_count", metrics.totalCount);
        aggregateMaxMetric(projectFile, "interfaces.direct_count", metrics.directCount);
        aggregateMaxMetric(projectFile, "interfaces.inherited_count", metrics.inheritedCount);
        aggregateMaxMetric(projectFile, "interfaces.complexity_score", metrics.complexityScore);

        logger.debug("Aggregated interface metrics on ProjectFile: {} (max_total={}, classes_analyzed={})",
                projectFile.getRelativePath(),
                projectFile.getDoubleProperty("interfaces.total_count.max"),
                projectFile.getIntegerProperty("interfaces.total_count.classes_analyzed"));
    }
